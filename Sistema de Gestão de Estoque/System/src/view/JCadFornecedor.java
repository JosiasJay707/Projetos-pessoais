...
public class JCadFornecedor extends javax.swing.JFrame {
    private int indice = 0;
    private Boolean valCRUD = false; // variável para verificar se o CRUD foi realizado
    FornecedorDAO selectFornecedores = new FornecedorDAO();
    List<MCadFornecedor> fornecedoresCad = new ArrayList<>();
    char confirmAcao = 'n';


    public JCadFornecedor() {
        initComponents();
...
fornecedoresCad = selectFornecedores.buscarFornecedor();

    txtIdFornecedor.setText(Integer.toString(fornecedoresCad.get(indice).getIdFornecedor()));
    txtRazaoSocial.setText(fornecedoresCad.get(indice).getRazaoSocial());
    txtApelido.setText(fornecedoresCad.get(indice).getApelido());
    txtCpfCnpj.setText(fornecedoresCad.get(indice).getCpfCnpj());
    txtTelefone.setText(fornecedoresCad.get(indice).getTelefone());
    txtEmail.setText(fornecedoresCad.get(indice).getEmail());
    txtCep.setText(fornecedoresCad.get(indice).getCep());
    txtEndereco.setText(fornecedoresCad.get(indice).getEndereco());
    txtBairro.setText(fornecedoresCad.get(indice).getBairro());
    txtNumero.setText(fornecedoresCad.get(indice).getNumero());
    txtCidade.setText(fornecedoresCad.get(indice).getCidade());
    cbPais.setSelectedItem(fornecedoresCad.get(indice).getPais());
    cbUF.setSelectedItem(fornecedoresCad.get(indice).getUf());
    // Caso tenha campo ou checkbox para status do fornecedor, ajustar aqui
    // Exemplo: cbxDesativado.setSelected(!fornecedoresCad.get(indice).isAtivo());
    ...
    private void initComponents() {
        ...
        btCadFornecedor.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagens/new_client_24x24.png"))); // NOI18N
        btCadFornecedor.setToolTipText("Cadastrar novo produto");
        ...
        jScrollPane3.setViewportView(txtIdFornecedor);
        ...
        jLabel17.setText("Cadastro de Fornecedor");
        ...
        btCadFornecedor.getAccessibleContext().setAccessibleDescription("");
...
//Botoes de confirmações
    ...
    private void btProximoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btProximoActionPerformed
        if(indice < fornecedoresCad.size() - 1){
            indice++;
            fornecedoresCad = selectFornecedores.buscarFornecedor(); // Atualiza a lista de fornecedores, se necessário
            ...
            cbUF.setSelectedItem(fornecedoresCad.get(indice).getUf());
            cbPais.setSelectedItem(fornecedoresCad.get(indice).getPais());
            cbxDesativado.setSelected(fornecedoresCad.get(indice).getDesativado());
        }
...
//Desabilitar os outros botões de CRUD
        btCadFornecedor.setEnabled(false);
        btReplicar.setEnabled(false);
        btExcluir.setEnabled(false);
//Comportamento dos campos
        txtRazaoSocial.setEditable(true);
...
//Botoes de confirmações
    ...
    if(confirmAcao == 'C'){
        ...
        fornecedor.setApelido(txtApelido.getText());
        ...
        fornecedor.setUf(ufSelecionada);
        String paisSelecionado = cbPais.getSelectedItem().toString();
        fornecedor.setPais(paisSelecionado);
        fornecedor.setDesativado(cbxDesativado.isSelected());
        ...
        if(txtRazaoSocial.getText().trim().length() < 2){
            ...
            valCRUD = true;
            indice = fornecedoresCad.size() - 1;
        }
    }

    if(confirmAcao == 'R'){  // Replicar fornecedor (mesmo que cadastrar, mas pode ter lógica diferente)
        FornecedorDAO dao = new FornecedorDAO();
        MCadFornecedor fornecedor = new MCadFornecedor();
    ...
    if(confirmAcao == 'A'){  // Alterar fornecedor
        FornecedorDAO dao = new FornecedorDAO();
        MCadFornecedor fornecedor = new MCadFornecedor();
        ...
        if(txtRazaoSocial.getText().trim().length() < 2){
            ...
            try {
                ...
                Logger.getLogger(JCadFornecedor.class.getName()).log(Level.SEVERE, null, ex);
            }
            valCRUD = true;
        }
    }

    if(confirmAcao == 'E'){  // Excluir fornecedor
        FornecedorDAO dao = new FornecedorDAO();
        MCadFornecedor fornecedor = new MCadFornecedor();
        fornecedor.setIdFornecedor(Integer.parseInt(txtIdFornecedor.getText()));
        dao.excluirFornecedor(fornecedor.getIdFornecedor());
        valCRUD = true;
        if(indice < fornecedoresCad.size() - 1){
            indice++;
        } else {
    ...
    btCadFornecedor.setEnabled(true);
    ...
    // Se houver checkbox para status do fornecedor, ajustar aqui
...
//Botoes de confirmações
    ...
    private void btUltimoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btUltimoActionPerformed
        fornecedoresCad = selectFornecedores.buscarFornecedor(); // Atualiza a lista de fornecedores, se necessário
        indice = fornecedoresCad.size() - 1; // posição do último elemento
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new JCadFornecedor().setVisible(true);
            }
        });
    }
